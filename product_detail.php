...
<head>
    ...
    <title>
        <?= htmlspecialchars($product['name']) ?> - Pixel Power
    </title>
    ...
    <div class="container" style="padding-top: 2rem; min-height: 80vh;">
        ...
        <div style="margin-bottom: 2rem; font-size: 0.9rem; color: var(--text-muted);">
            ...
            <a href="index.php?category=<?= urlencode($product['category']) ?>"
                style="color: var(--neon-blue); text-decoration: none;"><?= htmlspecialchars($product['category']) ?></a>
            /
            <?= htmlspecialchars($product['name']) ?>
        </div>

        <div
            style="display: flex; gap: 4rem; flex-wrap: wrap; background: rgba(255,255,255,0.02); padding: 3rem; border-radius: 20px; border: var(--glass-border);">
            ...
            <div style="flex: 1; min-width: 350px; display: flex; flex-direction: column;">
                ...
                <h1
                    style="font-size: 3.5rem; margin-bottom: 1.5rem; color: white; line-height: 1.1; text-transform: none; font-family: 'Inter'; font-weight: 700;">
                    <?= htmlspecialchars($product['name']) ?>
                </h1>

                <div style="display: flex; align-items: baseline; gap: 1rem; margin-bottom: 2rem;">
                    <span
                        style="font-size: 2.5rem; color: var(--neon-green); font-weight: 700;">$<?= number_format($product['price'], 2) ?></span>
                    <span
                        style="color: var(--text-muted); text-decoration: line-through; font-size: 1.2rem;">$<?= number_format($product['price'] * 1.2, 2) ?></span>
                </div>

                <div style="margin-bottom: 3rem;">
                    <h4
                        style="color: white; margin-bottom: 1rem; font-family: 'Orbitron'; font-size: 0.9rem; letter-spacing: 1px;">
                        ข้อมูลรายละเอียดสินค้า</h4>
                    <div
                        style="line-height: 1.8; color: var(--text-main); font-size: 1.05rem; background: rgba(0,0,0,0.2); padding: 1.5rem; border-radius: 10px; border-left: 3px solid var(--neon-blue);">
                        <?= nl2br(htmlspecialchars($product['description'])) ?>
                    </div>
                </div>

                <div
                    style="margin-top: auto; background: rgba(255,255,255,0.03); padding: 2rem; border-radius: 15px; border: 1px solid rgba(255,255,255,0.05);">
                    <form action="api.php" method="POST">
                        ...
                        <button type="submit" class="btn-neon"
                            style="padding: 1.2rem; border-radius: 50px; font-size: 1.1rem;">
                    ...
                    <p style="margin-top: 1.5rem; font-size: 0.85rem; color: var(--text-muted); text-align: center;">
                        <i class="fas fa-truck" style="color: var(--neon-blue);"></i> จัดส่งภายใน 3-5 วันทำการ |
